...
def aggregate_up_down_ratio_daily(df: pd.DataFrame, top: int = 100) -> tuple[list, str]:
    dt, list_html = aggregate_up_down_ratio(df, top)
    file_html = 'report/%04d/up_down_ratio_%02d-%02d.html' % (dt[0].year, dt[0].month, dt[0].day)

    return list_html, file_html


def aggregate_up_down_ratio_weekly(df: pd.DataFrame, top: int = 100) -> tuple[list, str]:
    dt, list_html = aggregate_up_down_ratio(df, top)
    file_html = 'report/%04d/up_down_ratio_weekly_%02d-%02d.html' % (dt[0].year, dt[0].month, dt[0].day)

    return list_html, file_html


def aggregate_up_down_ratio(df, top):
    # 列名の変更
